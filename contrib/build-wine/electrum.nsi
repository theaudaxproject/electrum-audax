...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-AUDAX"
  !define PRODUCT_WEB_SITE "https://github.com/theaudaxproject/electrum-audax"
  !define PRODUCT_PUBLISHER "Audax"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-audax-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-audax\electrum_audax\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-audax\*.*"
  File "c:\electrum-audax\electrum_audax\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe" 0


  ;Links audax: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\audax" "" "URL:audax Protocol"
  WriteRegStr HKCU "Software\Classes\audax" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\audax" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\audax\shell\open\command" "" "$\"$INSTDIR\electrum-audax-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\audax"
