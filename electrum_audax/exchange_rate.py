...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("AUDAX", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['AUDAX'][r]) for r in json['AUDAX']
                 if json['AUDAX'][r] is not None]  # Giving NULL for LTC
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=AUDAX")
        return json[ccy +'_AUDAX']


class Bitbank(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('public.bitbank.cc', '/audax_jpy/ticker')
...
class CoinMarketCap(ExchangeBase):
     ...
     def get_rates(self, ccy):
        json = self.get_json('api.coinmarketcap.com', '/v2/ticker/706/?convert=%s' % ccy)
...
class Bitmarket(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bitmarket.pl', '/json/AUDAXPLN/ticker.json')
...
class CryptoCompare(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('min-api.cryptocompare.com', '/data/price?fsym=AUDAX&tsyms=USD,EUR,THB,BTC,ETH')
...
class CoinMarketCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinmarketcap.com', '/v2/ticker/706/?convert=%s' % ccy)
        return {ccy.upper(): Decimal(json['data']['quotes'][ccy]['price'])}
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=AUDAX')
...
class Zaif(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.zaif.jp', '/api/1/last_price/audax_jpy')
        return {'JPY': Decimal(json['last_price'])}
