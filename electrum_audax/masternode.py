...
class MasternodePing(object):
    ...
    @classmethod
    def deserialize(cls, vds, protocol_version=70914):
...
class MasternodeAnnounce(object):
    ...
    Attributes:
        ...
        - vin: 2500 AUDAX input.
        - addr: Address that the masternode can be reached at.
        - collateral_key: Key that can spend the 2500 AUDAX input.
    ...
    def __init__(self, alias='', vin=None, addr=NetworkAddress(),
                 collateral_key='', private_key='', masternode_pubkey='', sig='', sig_time=0,
                 protocol_version=70914, last_ping=MasternodePing(),
                 announced=False):
        self.alias = alias
        if vin is None:
            vin = {'prevout_hash': '', 'prevout_n': 0,
    ...
    def from_dict(cls, d):
        kwargs = {}
        for key in ['alias', 'vin', 'collateral_key', 'private_key', 'masternode_pubkey', 'sig', 'sig_time',
    ...
    def dump(self):
        kwargs = {}
        for key in ['alias', 'vin', 'collateral_key', 'private_key', 'masternode_pubkey',
