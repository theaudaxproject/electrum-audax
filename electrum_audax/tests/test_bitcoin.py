...
from electrum_audax.bitcoin import (public_key_to_p2pkh, address_from_private_key,
                                   ...
                                   op_codes, base_encode, base_decode, BitcoinException)
from electrum_audax.bip32 import (BIP32Node, convert_bip32_intpath_to_strpath,
                                 xpub_from_xprv, xpub_type, is_xprv, is_bip32_derivation,
                                 is_xpub, convert_bip32_path_to_list_of_uint32,
                                 normalize_bip32_derivation)
from electrum_audax.crypto import sha256d, SUPPORTED_PW_HASH_VERSIONS
from electrum_audax import ecc, crypto, constants
from electrum_audax.ecc import number_to_string, string_to_number
from electrum_audax.util import bfh, bh2u, InvalidPassword
from electrum_audax.storage import WalletStorage
from electrum_audax.keystore import xtype_from_derivation

from electrum_audax import ecc_fast
...
class Test_bitcoin(SequentialTestCase):
    ...
    def test_msg_signing(self):
        ...
        self.assertEqual(sig1_b64, b'H728/TD0FJDC5GwYunKROiRe7uaUqUwwNq9GQhpXMVpgeLLiJ5NhluKNYXqqCFfVmfyhkGsM/ROmbi6lRTrH8ac=')
        self.assertEqual(sig2_b64, b'H9uiJF8QvlAqVSG8mE4+nm4qVP8+pgrmNmOLctBtj98QABHiLCgoDk76jn1z21yp7E61O1mtHToUXx05NNZHjx8=')
    ...
    def test_address_to_script(self):
        ...
        # TODO: AUDAX doesn't support segwit yet (check BIP141 / 143)
        ...
        # base58 P2PKH
        self.assertEqual(address_to_script('GTmh72MyvTrG9VafWxNj2dmsvUBcTrZ4HM'), '76a9146ce47b49c0c7004ffd0c61f8ad1b125f5c66cd0888ac')
        self.assertEqual(address_to_script('GYkSBnzLgUiA5poi4U4wz4WSexwpBuG7Eq'), '76a914a3803de8420a6d4567fd6f707598982e659ab28288ac')

        # base58 P2SH
        self.assertEqual(address_to_script('3T7gaNp2UCprx2pkwrcScuJbvw7rMG7AU2'), 'a91416e5649ae31f98287209961e9348eba12917c8d587')
        self.assertEqual(address_to_script('3kmcwzd5zb3TUkcuGCLhLTrVc3zGc9sx4h'), 'a914d88c4f5937c20a3f00a33ae7c0b8da73d2414b4a87')
...
class Test_bitcoin_testnet(TestCaseForTestnet):
    ...
    def test_address_to_script(self):
        ...
        # base58 P2PKH
        self.assertEqual(address_to_script('ggjc34YSjk7XgcuR888cbaK9GijoPHb7ZP'), '76a914a0482522a42d3ecd09b8d7a5a9b4f7f501737ea688ac')
        self.assertEqual(address_to_script('gnyWB7aK3JbMVuy8V2FtitCAWRgUPzGg82'), '76a914e4b9d4a050eed522dc32e673dacd8f72e1dd575988ac')

        # base58 P2SH
        self.assertEqual(address_to_script('69PX995h9fTcmmPeVD6Q3Vgvi7mHri8t6t'), 'a914c917cb6b71968a88683b774572590442c960335087')
        self.assertEqual(address_to_script('5vKirTcFXLbvc9UAzvug93cqrECRFvEMZQ'), 'a91439c645c8bef227c79880c4142b68e74087a92b8787')
...
class Test_xprv_xpub(SequentialTestCase):
    ...
    def test_is_bip32_derivation(self):
        ...
        self.assertTrue(is_bip32_derivation("m/44'/222'/0'/0/0"))
    ...
    def test_xtype_from_derivation(self):
        ...
        self.assertEqual('standard', xtype_from_derivation("m/44'/"))
        self.assertEqual('standard', xtype_from_derivation("m/44'/222'/0'"))
...
class Test_keyImport(SequentialTestCase):
    ...
    priv_pub_addr = (
           {'priv': '7r1rkeTrMx9ezWZvuZeLc5ow66i8cER1XaJw3yFRq64Ji3qmz3oB',
            'exported_privkey': 'p2pkh:7r1rkeTrMx9ezWZvuZeLc5ow66i8cER1XaJw3yFRq64Ji3qmz3oB',
            'pub': '03f0865dc8881b4c4a9a015f2e8bd8733eef4ca2f726cd1bc0bbb14c651be9e34e',
            'address': 'GLWJhLPeFE89xRCZzhDbofv1fPoHNRqcoW',
            'minikey' : False,
            'txin_type': 'p2pkh',
            'compressed': True,
            'addr_encoding': 'base58',
            'scripthash': '50d8f113984ed15ad18abe128f26beaaec33adf5a41f8ba6a3e42abafb48fd0d'},
           {'priv': 'p2pkh:7ubvKBCJn3W339diUBp1QZ3QA7oWK6zdqpgVpvyZMx3RL3BsnXdj',
            'exported_privkey': 'p2pkh:7ubvKBCJn3W339diUBp1QZ3QA7oWK6zdqpgVpvyZMx3RL3BsnXdj',
            'pub': '0349d0c703d5367b480b189e60832c0941229a4181ec02c3e9fcad928ef6ef43e4',
            'address': 'Gg6TqZkyzTMLqyaCmqDhEYqACiL7FoRPnJ',
            'minikey': False,
            'txin_type': 'p2pkh',
            'compressed': True,
            'addr_encoding': 'base58',
            'scripthash': 'f7d44779add63a68bceafb6af832b921b842a1e5e398099e4d331870cd09b4a5'},
    ...
    def test_public_key_from_private_key(self):
        for priv_details in self.priv_pub_addr:
            txin_type, privkey, compressed = deserialize_privkey(priv_details['exported_privkey'])
    ...
    def test_address_from_private_key(self):
        for priv_details in self.priv_pub_addr:
            addr2 = address_from_private_key(priv_details['exported_privkey'])
    ...
    def test_is_compressed_privkey(self):
        for priv_details in self.priv_pub_addr:
            self.assertEqual(priv_details['compressed'],
                             is_compressed_privkey(priv_details['exported_privkey']))
