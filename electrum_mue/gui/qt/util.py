...
from electrum_mue.i18n import _, languages
from electrum_mue.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_mue.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
...
def export_meta_gui(electrum_window, title, exporter):
    ...
    filename = electrum_window.getSaveFileName(_("Select file to save your {}").format(title),
                                               'electrum-mue_{}.json'.format(title), filter_)
    if not filename:
        return
    try:
        exporter(filename)
