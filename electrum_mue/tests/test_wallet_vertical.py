...
from electrum_mue import storage, bitcoin, keystore, bip32
from electrum_mue import Transaction
from electrum_mue import SimpleConfig
from electrum_mue.address_synchronizer import TX_HEIGHT_UNCONFIRMED, TX_HEIGHT_UNCONF_PARENT
from electrum_mue.wallet import sweep, Multisig_Wallet, Standard_Wallet, Imported_Wallet
from electrum_mue.util import bfh, bh2u
from electrum_mue.transaction import TxOutput
from electrum_mue.mnemonic import seed_type

from electrum_mue.plugins.trustedcoin import trustedcoin
...
class TestWalletKeystoreAddressIntegrityForMainnet(SequentialTestCase):
    ...
    def test_electrum_seed_2fa_legacy(self, mock_write):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '3UfjWsVWWQeCU1eWmM6tQvqiLKP8EgiRiB')
        self.assertEqual(w.get_change_addresses()[0], '3nzADjD491kKCoW957xPU123rgtwGsVeZy')
    ...
    def test_bip39_seed_bip44_standard(self, mock_write):
        ...
        ks = keystore.from_bip39_seed(seed_words, '', "m/44'/222'/0'")

        self.assertTrue(isinstance(ks, keystore.BIP32_KeyStore))

        self.assertEqual(ks.xprv, 'xprv9xnuTSuaxzuKEyKoP1krEt7jPFmPoPDjxX5SbAFCL1CLAU6SFsc2R5yGFZc8nu2mdubrXXsAMUpZMiryGTQMjARavCZ5fKY7UWphjpu2fTY')
        self.assertEqual(ks.xpub, 'xpub6BnFrxSUoNTcTTQGV3Hrc24TwHbtCqwbKk13PYeotLjK3GRaoQvGxtHk6rLsA4gU1EDR1bqpLmfKjmxngJVJg6KocarMCoMobkZCgAyip3s')

        w = WalletIntegrityHelper.create_standard_wallet(ks)
        self.assertEqual(w.txin_type, 'p2pkh')

        self.assertEqual(w.get_receiving_addresses()[0], 'GaskDPaDnZTBewkisXtX57LhP3txii4ntH')
        self.assertEqual(w.get_change_addresses()[0], 'GQQs4R2zxRZtojVxVCQ3cWy3rK7cRABGBr')
    ...
    def test_bip39_seed_bip44_standard_passphrase(self, mock_write):
        ...
        ks = keystore.from_bip39_seed(seed_words, UNICODE_HORROR, "m/44'/222'/0'")

        self.assertTrue(isinstance(ks, keystore.BIP32_KeyStore))

        self.assertEqual(ks.xprv, 'xprv9xhUSVvEEUoyrzoqDFVC6WBFKwPdSS6a3S7TvS457wuDsifYKtQRt5gRWFaY46nEVvoQeN6HKr9f9w6z7Z1Z1K1mEeaad7vVQQKMZj8METx')
        self.assertEqual(ks.xpub, 'xpub6Bgpr1T84rNH5UtJKH2CTe7ysyE7qtpRQf34ipTggHSCkWzgsRigRszuMZpE355VVFczs12zWHm8zxyihHVsKwcSrtDYyXa2dRXHFzGAhsg')

        w = WalletIntegrityHelper.create_standard_wallet(ks)
        self.assertEqual(w.txin_type, 'p2pkh')

        self.assertEqual(w.get_receiving_addresses()[0], 'Ge1ByyKGYTo4Tk6PNMTMZFtzW3Z37qCcwJ')
        self.assertEqual(w.get_change_addresses()[0], 'Ggf6BHt5xaGmJFjU7RpSsWHvmi3K7hT9uX')
    ...
    def test_electrum_multisig_seed_standard(self, mock_write):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '3S5K2X3TPhqrvEPtT5v5TLKmVWnUNnbiPJ')
        self.assertEqual(w.get_change_addresses()[0], '3Vs7vLb9ZffDc8sQ6nBoyQ3FvBxntje3VL')
    ...
    def test_bip39_multisig_seed_bip45_standard(self, mock_write):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '3hj4P961bgRPzjA4imyYRQNcgWNRXhUC2h')
        self.assertEqual(w.get_change_addresses()[0], '3ecaD2FyBQEudshj7NVcm9FGZwteeL3qt4')
    ...
    def test_bip32_extended_version_bytes(self, mock_write):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'GSWRecuUpXuYLFQbUKWDEgJsejku25Jt9Q')
        self.assertEqual(w.get_change_addresses()[0], 'GX5SXktyzhPvdSwcZY1CWrGuuosXe7rXaf')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '3TmmSByYxFwT18qXaZAZQ3ELZ4HZTunUyY')
        self.assertEqual(w.get_change_addresses()[0], '3YFPTSrsN4u9Fm4JyXfBPsxvdGPK8KZpmV')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'mue1qtuynwzd0d6wptvyqmc6ehkm70zcamxpsnt6v32')
        self.assertEqual(w.get_change_addresses()[0], 'mue1qjy5zunxh6hjysele86qqywfa437z4xwmmkcht9')

        ks = create_keystore_from_bip32seed(xtype='standard')  # p2sh
        w = WalletIntegrityHelper.create_multisig_wallet([ks], '1of1')
        self.assertEqual(ks.xprv, 'xprv9s21ZrQH143K3nyWMZVjzGL4KKAE1zahmhTHuV5pdw4eK3o3igC5QywgQG7UTRe6TGBniPDpPFWzXMeMUFbBj8uYsfXGjyMmF54wdNt8QBm')
        self.assertEqual(ks.xpub, 'xpub661MyMwAqRbcGH3yTb2kMQGnsLziRTJZ8vNthsVSCGbdBr8CGDWKxnGAFYgyKTzBtwvPPmfVAWJuFmxRXjSbUTg87wDkWQ5GmzpfUcN9t8Z')
        self.assertEqual(w.get_receiving_addresses()[0], '3eQPkEoCVmaemw4vitiLXFfLK9HeVD6nbS')
        self.assertEqual(w.get_change_addresses()[0], '3mULuRa4gAxA3ovBCVKn7h9pzeri84XzoX')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '3jLnwSdT9Sn6reZ6tLBtztKVCNxCuHsRLT')
        self.assertEqual(w.get_change_addresses()[0], '3mYsbhrnEeHTDwZnkKpiJf9AWZvxuxEtqe')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'mue1q84x0yrztvcjg88qef4d6978zccxulcmc9y88xcg4ghjdau999x7q76l7qz')
        self.assertEqual(w.get_change_addresses()[0], 'mue1q0fj5mra96hhnum80kllklc52zqn6kppt3hyzr49yhr3ecr42z3tsrwm9gt')
...
class TestWalletKeystoreAddressIntegrityForTestnet(TestCaseForTestnet):
    ...
    def test_bip32_extended_version_bytes(self, mock_write):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'gancj8nmRMi7QFmnwVVLLCe5R1DVfJJ5By')
        self.assertEqual(w.get_change_addresses()[0], 'gfMdcGnGbXCVhTJp2hzKcNc7g5L8KEsDeC')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '5soPLqmHDLihujg3j5AUJns4L5qDhRbRS4')
        self.assertEqual(w.get_change_addresses()[0], '5xH1N6ebd9gQAMtq83f6JdbeQHvyTBBAdh')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'muet1qtuynwzd0d6wptvyqmc6ehkm70zcamxpsymatr6')
        self.assertEqual(w.get_change_addresses()[0], 'muet1qjy5zunxh6hjysele86qqywfa437z4xwmvxlse4')

        ks = create_keystore_from_bip32seed(xtype='standard')  # p2sh
        w = WalletIntegrityHelper.create_multisig_wallet([ks], '1of1')
        self.assertEqual(ks.xprv, 'tprv8ZgxMBicQKsPecD328MF9ux3dSaSFWci7FNQmuWH7uZ86eY8i3XpvjK8KSH8To2QphiZiUqaYc6nzDC6bTw8YCB9QJjaQL5pAApN4z7vh2B')
        self.assertEqual(ks.xpub, 'tpubD6NzVbkrYhZ4Y5Epun1qZKcACU6NQqocgYyC4RYaYBMWw8nuLSMR7DvzVamkqxwRgrTJ1MBMhc8wwxT2vbHqMu8RBXy4BvjWMxR5EdZroxE')
        self.assertEqual(w.get_receiving_addresses()[0], '64S1etavkrMugXuSsQiFS1J46AqJnCa8sM')
        self.assertEqual(w.get_change_addresses()[0], '6BVxp5MnwFjQxQkhM1Kh2SnYmgQNQiwhXK')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], '69NQr6RBQXZMmFPd2rBoudxCyQVsBSfLAs')
        self.assertEqual(w.get_change_addresses()[0], '6BaVWMeWVj4i8YQJtqpdDQmtHbUdDY2sm1')
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'muet1q84x0yrztvcjg88qef4d6978zccxulcmc9y88xcg4ghjdau999x7qrvjepz')
        self.assertEqual(w.get_change_addresses()[0], 'muet1q0fj5mra96hhnum80kllklc52zqn6kppt3hyzr49yhr3ecr42z3ts7ckzft')
...
class TestWalletSending(TestCaseForTestnet):
    ...
    def test_sending_between_p2sh_2of3_and_uncompressed_p2pkh(self, mock_write):
        wallet1a = WalletIntegrityHelper.create_multisig_wallet(
            [
                ...
                keystore.from_xpub('xpub661MyMwAqRbcFDD48yabbo2japn2uTLbo8tyVnKkjcgq7UhjVRrwtZt4S9VUYjN4cWtc8H8ysGrwdJ2iXhyHEenERifuDZi9J4q3idY84KP'),
                keystore.from_xpub('xpub661MyMwAqRbcFDRuTuRNhpvgbWBoL6oe7hd4Agqcs9MWNRAeDa7kQ55ogC6gkaMfMrd343ZNuqVbrmSLattKYCv1q771fXU1eoQbjSuGGxS')
            ],
            '2of3', gap_limit=2
        )
    ...
    def test_sweep_p2pk(self):
        ...
        privkeys = ['p2pkh:93NQ7CFbwTPyKDJLXe97jczw33fiLijam2SCZL3Uinz1NSbHrTu', ]
        network = NetworkMock()
...
class TestWalletOfflineSigning(TestCaseForTestnet):
    ...
    def test_sending_offline_hd_multisig_online_addr_p2sh(self, mock_write):
        ...
        wallet_online = WalletIntegrityHelper.create_imported_wallet(privkeys=False)
        wallet_online.import_address('GTmh72MyvTrG9VafWxNj2dmsvUBcTrZ4HM')
