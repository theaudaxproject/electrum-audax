...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("MUE", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['MUE'][r]) for r in json['MUE']
                 if json['MUE'][r] is not None]  # Giving NULL for LTC
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=MUE")
        return json[ccy +'_MUE']


class Bitbank(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('public.bitbank.cc', '/mue_jpy/ticker')
...
class Bitmarket(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bitmarket.pl', '/json/MUEPLN/ticker.json')
...
class CryptoCompare(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('min-api.cryptocompare.com', '/data/price?fsym=MUE&tsyms=USD,EUR,THB,BTC,ETH')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=MUE')
...
class Zaif(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.zaif.jp', '/api/1/last_price/mue_jpy')
        return {'JPY': Decimal(json['last_price'])}
