...
from electrum_mue.util import bfh, bh2u, UserCancelled, UserFacingException
...
class KeepKeyPlugin(HW_PluginBase):
    ...
    def sign_transaction(self, keystore, tx, prev_tx, xpub_path):
        ...
        signatures = client.sign_tx(
            self.get_coin_name(), inputs, outputs, lock_time=tx.locktime)[0]
            ...
            if txin['type'] == 'coinbase':
                ...
                if for_sig:
                    ...
                    if len(x_pubkeys) == 1:
                        ...
                        multisig = self.types.MultisigRedeemScriptType(
                            pubkeys=pubkeys,
                            signatures=map(lambda x: bfh(
                                x)[:-1] if x else b'', txin.get('signatures')),
                            m=txin.get('num_sig'),
                        )
    ...
    def tx_outputs(self, derivation, tx):
        ...
        def create_output_by_derivation():
            ...
            if len(xpubs) == 1:
                ...
                pubkeys = [self._make_node_path(
                    xpub, address_n) for xpub in xpubs]
                multisig = self.types.MultisigRedeemScriptType(
                    pubkeys=pubkeys,
